...
try {
    ...
    // If action is to check schemas, output all schemas and exit
    if ($action === 'check_all_schemas') {
        ...
        foreach ($tables as $tableName) {
            ...
            echo "──────────────────────────────────────────────────────────\n";
            printf("%-5s %-30s %-20s %-8s %-10s\n", 
                   "No.", "Field", "Type", "Null", "Key");
            echo "──────────────────────────────────────────────────────────\n";
            
            foreach ($columns as $i => $col) {
                printf("%-5d %-30s %-20s %-8s %-10s\n",
                       $i + 1,
                       $col['Field'],
                       $col['Type'],
                       $col['Null'],
