...
<div class="container-fluid">
    <div class="row mb-4">
        ...
        <div class="col-auto">
            <a href="<?= BASE_URL ?>/?page=financeiro&action=conta-pagar-create" class="btn btn-primary">
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-3">
            <div class="card bg-success text-white">
                <div class="card-body">
                    <h3>R$ <?= number_format($stats['total_pago_mes'] ?? 0, 2, ',', '.') ?></h3>
                    <p class="mb-0">Pago no Mês</p>
                </div>
            </div>
        </div>
        <div class="col-md-3">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form method="GET" action="<?= BASE_URL ?>/">
                ...
                <input type="hidden" name="action" value="contas-pagar">
                <div class="row">
                    <div class="col-md-3">
                        <label>Buscar</label>
                        <input type="text" name="busca" class="form-control" value="<?= htmlspecialchars($filtros['busca'] ?? '') ?>" placeholder="Descrição ou documento...">
                    </div>
                    <div class="col-md-2">
                        <label>Status</label>
            ...
            <div class="table-responsive">
                <table class="table table-hover" id="tabelaContas">
                    ...
                    <tbody>
                        <?php if (empty($contas)): ?>
                        <tr>
                            <td colspan="11" class="text-center text-muted">Nenhuma conta encontrada</td>
                        </tr>
                        <?php else: ?>
                            <?php foreach ($contas as $conta): 
                                ...
                                $saldo = $conta['valor_original'] - $conta['valor_pago'];
                            ?>
                            <tr>
                                <td><?= htmlspecialchars($conta['numero_documento']) ?></td>
                                <td><?= htmlspecialchars($conta['descricao']) ?></td>
                                <td><?= htmlspecialchars($conta['fornecedor_nome'] ?? '-') ?></td>
                                ...
                                <td class="text-end">R$ <?= number_format($conta['valor_pago'], 2, ',', '.') ?></td>
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <a href="<?= BASE_URL ?>/?page=financeiro&action=conta-pagar-show&id=<?= $conta['id'] ?>" class="btn btn-primary" title="Ver detalhes">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <?php if ($conta['status'] === 'pendente' || $conta['status'] === 'vencida'): ?>
                                        <button type="button" class="btn btn-success" onclick="modalPagar(<?= $conta['id'] ?>)" title="Pagar">
                    ...
                    <tfoot>
                        <tr class="table-secondary fw-bold">
                            ...
                            <td class="text-end">R$ <?= number_format(array_sum(array_column($contas, 'valor_pago')), 2, ',', '.') ?></td>
                            <td class="text-end">R$ <?= number_format(array_sum(array_map(fn($c) => $c['valor_original'] - $c['valor_pago'], $contas)), 2, ',', '.') ?></td>
                            <td colspan="2"></td>
                        </tr>
                    </tfoot>
                </table>
            ...
            <nav class="mt-3">
                <ul class="pagination justify-content-center">
                    ...
                    <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                        <a class="page-link" href="<?= BASE_URL ?>/?page=financeiro&action=contas-pagar&p=<?= $i ?><?= !empty($filtros) ? '&' . http_build_query($filtros) : '' ?>">
                            <?= $i ?>
                        </a>
                    </li>
                    <?php endfor; ?>
...
<div class="modal fade" id="modalPagamento" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST" action="<?= BASE_URL ?>/?page=financeiro&action=conta-pagar-pagar">
                <input type="hidden" name="csrf_token" value="<?= $_SESSION['csrf_token'] ?? '' ?>">
                <input type="hidden" name="id" id="pagamento_conta_id">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="valor_pago" class="form-control" step="0.01" min="0" required>
                    </div>
                    <div class="mb-3">
...
<script>
function modalPagar(contaId) {
    document.getElementById('pagamento_conta_id').value = contaId;
